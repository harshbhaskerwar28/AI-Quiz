    ...
    return (
      <Card className={`w-full backdrop-blur-lg ${isDarkMode ? 'bg-gray-900/50 text-white' : 'bg-white/50 text-gray-800'} border-gray-800`}>
        <CardContent className="flex flex-col items-center justify-center h-64">
          ...
          <Button onClick={() => window.location.reload()} className="mt-4 bg-gradient-to-r from-red-500 to-pink-500 text-white font-bold py-2 px-4 rounded-full">
            Restart Quiz
          </Button>
        </CardContent>
      </Card>
            ...
            {currentQuestionData.options.map((option, index) => (
              ...
              >
                <Button
                  ...
                  } ${isDarkMode ? 'text-white' : 'text-gray-800'} transition-colors duration-300 overflow-hidden`}
                >
                  <div className="flex items-center w-full">
                    ...
                    {selectedAnswer === index && (
                      ...
                      >
                        {option === currentQuestionData.correctAnswer ? (
                          <CheckCircle className="inline-block w-6 h-6 text-white" />
                        ) : (
                          <XCircle className="inline-block w-6 h-6 text-white" />
                        )}
                      </motion.span>
                    )}
                  </div>
      ...
      <CardFooter>
        ...
        >
          <Button
            ...
            className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 text-white text-lg md:text-xl py-4 md:py-6 rounded-full font-bold"
          >
